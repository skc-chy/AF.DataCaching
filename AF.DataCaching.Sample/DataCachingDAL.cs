...
using Architecture.Foundation.DataCaching;
        ...
        public List<DataCachingEntity> GetEmployeeList()
        {
            ...
            if (empList == default(List<DataCachingEntity>) || empList.Count == 0)
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                        empList.Add(new DataCachingEntity { EmpID = new Guid(reader["EmpID"].ToString()), Name = reader["Name"].ToString(), Address = reader["Address"].ToString(), EMail = reader["EMail"].ToString(), Phone = reader["Phone"].ToString() });
